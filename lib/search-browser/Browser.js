...
var Browser = (function (_Component) {
  ...
  Browser.prototype.render = function render() {
    ...
    var inlineStyle = { top: this.props.top };
  ...
  Browser.prototype.renderTabs = function renderTabs() {
    if (!this.state.tabs || !this.state.search) return null;
    return _react2['default'].createElement(_tabsTabsWithControls2['default'], { data: this.state.tabs, onSelect: this.onSelectTab.bind(this) });
  };
  ...
  Browser.prototype.renderContent = function renderContent() {
    ...
    if (!data) return null;
    if (!this.state.search && _lodashLangIsEmpty2['default'](this.state.filters)) return null;
